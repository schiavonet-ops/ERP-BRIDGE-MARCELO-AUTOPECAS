...
def atualizar_produto(codigo: int, dados: ProdutoAtualizar):
    ...
    try:
        ...
        if dados.preco_sem_lucro is not None:
            campos_e.append("EST_VENDACUSTO = ?"); valores_e.append(dados.preco_sem_lucro); atualizados.append("preco_sem_lucro")
        if dados.preco_sugerido is not None:
            campos_e.append("EST_VENDASUGERIDO = ?"); valores_e.append(dados.preco_sugerido); atualizados.append("preco_sugerido")
        if dados.preco_venda is not None:
            campos_e.append("EST_VENDA = ?"); valores_e.append(dados.preco_venda); atualizados.append("preco_venda")
        if dados.margem is not None:
            campos_e.append("EST_MARGEM = ?"); valores_e.append(dados.margem); atualizados.append("margem")
        if dados.perc_comissao is not None:
            campos_e.append("EST_PERCCOMISSAO = ?"); valores_e.append(dados.perc_comissao); atualizados.append("perc_comissao")
        if dados.perc_imposto is not None:
            campos_e.append("EST_PERCIMPOSTO = ?"); valores_e.append(dados.perc_imposto); atualizados.append("perc_imposto")
        if dados.perc_fixo is not None:
            campos_e.append("EST_PERCFIXO = ?"); valores_e.append(dados.perc_fixo); atualizados.append("perc_fixo")
        if dados.perc_outros is not None:
            campos_e.append("EST_PERCOUTROS = ?"); valores_e.append(dados.perc_outros); atualizados.append("perc_outros")
...
def produto_completo(codigo: int):
    ...
    try:
        ...
        return {
            "codigo": row[0], "descricao": s(row[1]), "codigo_proprio": s(row[2]),
            "cod_barras": s(row[3]), "localizacao": s(row[4]),
            "grupo_codigo": row[5], "grupo_nome": nome_aux("GRUPO","GRU_CODIGO","GRU_NOME", row[5]),
            "secao_codigo": row[6], "secao_nome": nome_aux("SECAO","SEC_CODIGO","SEC_NOME", row[6]),
            "marca_codigo": row[7], "marca_nome": nome_aux("MARCA","MAR_CODIGO","MAR_NOME", row[7]),
            "aplicacao": aplicacao, "conversao": conversao, "memo_raw": memo_raw,
            "ncm": s(row[9]), "cod_barras2": s(row[10]),
            "preco_venda": f(row[11]), "preco_sugerido": f(row[12]),
            "custo": f(row[13]), "custo_medio": f(row[14]),
            "margem": f(row[15]), "perc_fixo": f(row[16]),
            "perc_imposto": f(row[17]), "perc_comissao": f(row[18]),
            "perc_outros": f(row[19]), "desconto_max": f(row[20]),
            "estoque_atual": f(row[21]), "estoque_minimo": f(row[22]),
        }
    except HTTPException:
        raise
    except Exception as e:
...
@app.post("/sync/delta")
def sync_delta():
    """Sync delta de produtos + NFs — chamado pelo cron a cada 15 min."""
    s = db.status_sync()
...
def sync_nfs(completo: bool = Query(False)):
    ...
    delta = None if completo else (datetime.now() - timedelta(days=1)).date()
    ...
    try:
        ...
        cur.execute(f"""
            SELECT FIRST {limit}
                ...
                m.MOV_QTDE, m.MOV_ESTOQUE, m.MOV_VALORUNI,
                m.MOV_VALORTOTAL, m.MOV_ORIGEM,
                m.MOV_NOTAENTRADA, m.MOV_NOTASAIDA, m.MOV_MEMO
            FROM MOVESTOQUE m
            WHERE m.MOV_PRODUTO = ?
        ...
        for row in movimentos_raw:
            ...
            os_id = cond_id = None
            tipo = "Ajuste"
            documento = numero_doc = cliente_nome = ""
            cliente_codigo = None
            detalhes = {}
                ...
                try:
                    ...
                    if r:
                        ...
                        if r[7]:
                            ...
                            os_id = r[7]
                            detalhes = {"numero_nota": numero_doc, "numero_os": r[7], "status_os": r[8], "quilometragem": r[9], "veiculo_placa": _s(r[10]), "veiculo_modelo": _s(r[11]), "valor_total": _f(r[4])}
                        elif r[12]:
                            tipo = "Condicional"
                            cond_id = r[12]
                            detalhes = {"numero_nota": numero_doc, "numero_condicional": r[12], "status_condicional": r[13], "data_prevista": str(r[14]) if r[14] else "", "valor_total": _f(r[4])}
                        else:
                            detalhes = {"numero_nota": numero_doc, "valor_total": _f(r[4])}
                except Exception:
                    pass
                ...
                try:
                    ...
                    if r:
                        ...
                        if "bridge" in descricao_entrada.lower():
                            tipo = "Ajuste Bridge"
                        detalhes = {"numero_entrada": numero_doc, "descricao": descricao_entrada, "valor_total": _f(r[4])}
                except Exception:
                    pass
